...
class CategoricalLoss(nn.Module):
    def __init__(self, atoms, v_max, v_min):
        super(CategoricalLoss, self).__init__()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.atoms = atoms
        self.v_max = v_max
        self.v_min = v_min
        self.supports = torch.linspace(v_min, v_max, atoms).view(1, 1, atoms).to(self.device) # RL: [bs, #action, #quantiles]
    ...
    def forward(self, anchor, feature, skewness):
