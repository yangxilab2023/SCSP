...
######################################################################5.13
def define_A(input_nc,output_nc, ngf, netA, norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, no_antialias_up=False, gpu_ids=[], opt=None):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)

    if netA == 'resnet_1blocks':
        # net = CoordAtt(input_nc, out,output_nc, reduction=32,norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, opt=opt)
        net = ResnetGenerator1(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=1, opt=opt)
    elif netA == 'resnet_3blocks':
        net = ResnetGenerator1(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=3, opt=opt)
    elif netA == 'resnet_6blocks':
        net = ResnetGenerator1(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=6, opt=opt)
    elif netA == 'resnet_9blocks':
        net = ResnetGenerator1(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
    # elif netA == 'resnet_6blocks':
    #     net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=6, opt=opt)
    # elif netA == 'resnet_4blocks':
    #     net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=4, opt=opt)
    # elif netA == 'unet_128':
    #     net = UnetGenerator(input_nc, output_nc, 7, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    # elif netA == 'unet_256':
    ...
    # elif netA == 'stylegan2':
    #     net = StyleGAN2Generator(input_nc, output_nc, ngf, use_dropout=use_dropout, opt=opt)
    # elif netA == 'smallstylegan2':
    #     net = StyleGAN2Generator(input_nc, output_nc, ngf, use_dropout=use_dropout, n_blocks=2, opt=opt)
    # elif netA == 'resnet_cat':
    #     n_blocks = 8
    #     net = G_Resnet(input_nc, output_nc, opt.nz, num_downs=2, n_res=n_blocks - 4, ngf=ngf, norm='inst', nl_layer='relu')
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % netA)
    # net = nn.Sequential(net, Norm())######
    return init_net(net, init_type, init_gain, gpu_ids, initialize_weights=('stylegan2' not in netA))
...
def calc_Style_Loss(features, targets, weights=None):
    if weights is None:
        weights = [1/len(features)] * len(features)
        
    style_loss = 0
    for f, t, w in zip(features, targets, weights):
        style_loss += mse_criterion(gram(f), gram(t)) * w
    return style_loss
...
class PerpetualLoss(nn.Module):
    ...
    def forward(self, x, y, fake):              
        x_vgg, y_vgg, fake_vgg = self.vgg(x), self.vgg(y), self.vgg(fake)
        content_loss = calc_Content_Loss(x_vgg, fake_vgg)
        style_loss = calc_Content_Loss(y_vgg, fake_vgg)   
...
class CoordAtt(nn.Module):
    def __init__(self, inp, oup, reduction=32):
        ...
        self.convsingle = nn.Conv2d( oup,1, kernel_size=1, stride=1, padding=0)
...
class ResnetGenerator1(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=3, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        assert(n_blocks >= 0)
        ...
        for i in range(n_blocks):       
            model += [CoordAtt(ngf * mult, ngf * mult, reduction=32)]
